...
import type { ToggleSwitchRootProps } from 'primereact/toggleswitch'
import { ToggleSwitch as ToggleSwitchPrime } from 'primereact/toggleswitch'
...
function ToggleSwitch({
  ...
  children,
  ...props
}: ToggleSwitchRootProps & { children?: ReactNode }) {
  return (
    <ToggleSwitchPrime.Root
      invalid={invalid}
      className={cn('relative inline-block w-9 h-5.5', className)}
      inputClassName="peer cursor-pointer disabled:cursor-default appearance-none absolute top-0 inset-s-0 w-full h-full m-0 p-0 opacity-0 z-10 rounded-full"
      aria-invalid={invalid}
      {...props}
    >
      <ToggleSwitchPrime.Control
        aria-invalid={invalid}
